...
function volunteerBase() {
    ...
    //this.seqArr = ["冲", "稳", "保", "垫"];
    this.seqArr = ['<div class="volun volun-chong">冲</div>',
        '<div class="volun volun-wen">稳</div>',
        '<div class="volun volun-bao">保</div>',
        '<div class="volun volun-dian">垫</div>',];
...
volunteerBase.prototype.getUniversityName = function () {
    ...
    util.ajaxFun(util.INTERFACE_URL.getVolunteerSchool, 'GET', {
        ...
        if (res.rtnCode == '0000000') {
            var universityInfoViewListObj = res.bizData.universityInfoViewList;

            self.provinceSelectKeyList = universityInfoViewListObj;
            var universityList = universityInfoViewListObj;
            if (universityList) {
...
volunteerBase.prototype.checkFirstBind = function (t) {
    ...
    $(".checkFirst" + t).click(function () {
        ...
        $('#province-alert-select').val('所在地区');
...
volunteerBase.prototype.getMajorFirstData = function () {
    var self = this;
    var subjectFirst = util.getLinkey('f');
    if (subjectFirst == 'true') {
        util.ajaxFun(util.INTERFACE_URL.getVolunteerSchool, 'GET', {
            "score": score,
            "batch": batch,
            "province": province,
            ...
            if (res.rtnCode == '0000000') {
                var universityList = res.bizData.universityInfoViewList;
                for (var i = 0; i < universityList.length; i++) {
...
/**
 * 选择院校按钮事件|搜索框下拉省份筛选|策略梯度提示
 */
volunteerBase.prototype.maxSchoolBtnEvent = function () {
    var self = this;
    var subjectFirst = util.getLinkey('f');
    for (var j = 0; j < $(".sel-school-btn").length; j++) {
        $(".sel-school-btn").eq(j).unbind("click").bind("click", {data: j}, function (e) {
            //使用dom确定第几个志愿
            var strategyId = e.data.data;
            var configData = require('volunteerSchoolProfessConf'),
                batchIndex = {};
            var type = $(this).attr("datatypet");
            var TIPSCONSTANTICON = configData.config[province].subProfessData['subData_' + batch.split('-')[0]].tipIcon,
                iconArr = TIPSCONSTANTICON.split('');
            $.each(iconArr, function (i, v) {
                if (v == '冲') iconArr[i] = "<div class='volun volun-chong'>冲</div>";
                if (v == '稳') iconArr[i] = "<div class='volun volun-wen'>稳</div>";
                if (v == '保') iconArr[i] = "<div class='volun volun-bao'>保</div>";
                if (v == '垫') iconArr[i] = "<div class='volun volun-dian'>垫</div>";
            });
            self.divideUniversity(self.outPutArr, type);
            var tempDialog = handleBars.compile(self.$tempVolunteerDialog.html());
            //判断是否是二号模板[兼容福建北京40,20个志愿的梯度提示]
            var pathArr = (window.location.pathname).split('.');
            var tplCode = pathArr[0].substring(pathArr[0].length, pathArr[0].length - 1), headerHTML = '';
            if (tplCode == 1) {  //1号模板梯度策略提示
                $.each(configData.volunteerSchoolProfessFun().schoolArr, function (i, v) {
                    batchIndex[v] = i;
                });
                headerHTML = tempDialog({t: type, iconType: iconArr[batchIndex[type]]});
            } else if (tplCode == 3) { //3号模板梯度策略提示
                var catchTxt = ['第一志愿', '第二志愿', '第三志愿', '第四志愿', '第五志愿'];
                var tt = catchTxt[parseInt(type.split('_')[1])] + type.split('_')[0] + '院校的';  //A_0
                headerHTML = tempDialog({t: type, batchT: tt, iconType: iconArr[strategyId]});
            } else {
                headerHTML = tempDialog({t: type, iconType: iconArr[type - 1]});  //2号模板梯度策略提示
            }
             ...
             * 搜索框下拉省份筛选
             * */
            var selectProvinceTpl = '<option value="00">所在地区' + '</option>', arrAreaName = [], arrAreaId = [];
            ...
            $.each(self.provinceSelectKeyList, function (i, v) {
                arrAreaName.push(v.areaName);
                arrAreaId.push(v.areaId);
            });
            for (var i = 0; i < arrAreaName.unique().length; i++) {
                selectProvinceTpl += '<option value=' + (arrAreaId.unique())[i] + '>' + (arrAreaName.unique())[i] + '</option>';
            }
            $('#province-alert-select').html(selectProvinceTpl);
            if (subjectFirst == 'true') {
                $('.tab-alert-toggle').show();  //专业优先切换
            }
        });
    };


    $(document).on('change', '#province-alert-select', function () {
        //获取volunteerSchoolList下面的所有tr
        var _thisV = $(this).val();
        var areaCode = $('#volunteerSchoolList tr .selectAreaKey');
        $.each(areaCode, function (i, v) {
            $(v).attr('areaId') == _thisV ? $(v).parent().show() : $(v).parent().hide();
        });
        if ($(this).val() === '00') {
            $('#volunteerSchoolList tr').show()
        };
    });
};
...
volunteerBase.prototype.checkUniversity = function (univerObj, type) {
    ...
    $(".school-result-btn" + type).click(function (event) {
        ...
        if (!!$university) {
            //获取智高考排名
            util.ajaxFun(util.INTERFACE_URL.getPredictZgkRank, 'get', {
                "score": util.cookie.getCookieValue('volunteerScore'),
                "uid": $university.universityId
            }, function (res) {
                if (res.rtnCode == '0000000') {
                    $university.rank = res.bizData.rank;
                }
...
volunteerBase.prototype.saveReport = function () {
    ...
    $("#volunteer-btn").click(function () {
        ...
        //提交专业个数限制.
        //var a = $('.specialty-click' + tag + ' .input').length, b = 0;
        //for (var i = 0; i < a; i++) {
        //    if ($('.specialty-click' + tag + ' .majorRep').eq(i).val() == '') {
        //        b++;
        //    }
        //}
        //if (b == a) {
        //    tips('#tips', "一个志愿请至少填写一个专业");
        //    return false;
        //}
...
volunteerBase.prototype.saveReportResult = function () {
    ...
    if (self.FlagResult) {
        ...
        if (province == 'js') {
            //选测等级
            data.extendProper = util.cookie.getCookieValue('objLevel')
        }
        var callback = function (res) {
            if (res.rtnCode == '1000001') {
                tips('#tips', res.msg);
            }
            if (res.rtnCode == '1000004') {
                tips('#tips', res.msg);
...
/**
 * volunteer-special-tips
 * 选择院校提示
 * 黑龙江\山西\天津\吉林
 * @type {volunteerBase}
 */
var volunteerSpecialProvince = ['hl', 'sx', 'tj', 'jl'];
if ($.inArray(province, volunteerSpecialProvince) > -1) {
    $('.volunteer-special-tips').show();
};
