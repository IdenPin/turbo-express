 ...
 * “智高考”报名人数、排名为空时显示
 * */
define(['commonjs', 'tips', 'handlebars', 'cookie', 'timeFormat', 'printArea', '../css/volunteer/predict-result-tpl0'], function (util, tips, handlebars, cookie, getTime, printArea) {
    //路由判断
    ...
    handlebars.registerHelper('controlLine', function (data) {
        ...
        for (var i = 0; i < provinceYearLine.length; i++) {
            strTpl += "<li><i class='icon-item-li'></i>" + nowYear + '年本省' + upper[i] + '省控线：' + provinceYearLine[i] + "分</li>";
        }
        return strTpl;
    });
    //filter-录取率
